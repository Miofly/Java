...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
	...
	renderNumberCellHtml: function(date) {
		...
		if (this.view.dayNumbersVisible) {
			...
			if (fes && fes.length > 0) {
				html += this.view.buildGotoAnchorHtml(date, { 'class': 'fc-day-number' }, date.date() )+ "<div class='fc-day-cnTerm'>" + $.trim(fes[0].desc) + "</div>";
			}
			else if (cTerm) {
				html += this.view.buildGotoAnchorHtml(date, { 'class': 'fc-day-number' }, date.date() )+ "<div class='fc-day-cnTerm'>" + cTerm + "</div>"
			}
			else{
				html += this.view.buildGotoAnchorHtml(date, { 'class': 'fc-day-number' }, date.date() );
			}

			// 无节气和节日时显示农历
			// if (!cTerm && (!fes || fes.length == 0)) {
			// 	html += "<div class='fc-day-cnDate'>" + lunar(date).lMonth
			// 		+ "月" + lunar(date).lDate + "</div>";
			// }
		}
