...
function mapShow(selectCityname,ele){
    ...
    /*市级地图数据*/
    ajaxPostPre(getRootPath()+"/map/citys?bareaName="+selectCityname,cityDatack);
    /*市级地图回调函数*/
...
function baiduMapData(selectCityname,cityname){
    ...
    /*中心点坐标，以及区县名称*/
    ajaxPostPre(getRootPath()+"/map/citysx?bareaName="+cityname,bdMapCenterCk);
...
function baiduMap(result,selectCityname,cityname) {
    ...
    /*营业厅的坐标以及分数数据*/
    ajaxPostPre(getRootPath()+"/map/cityx?distName="+selectCityname+cityname,YytDataCk);
...
function gj(cityname){
    ...
    function gjCk(result){
        $.each(result, function (i, data) {
            $('.mio-scrollNews').append('<li style="color: red">'+data.ALARM_CONTENT+'</li>');
        });
    }
}
...
function dsywpm(){
    ...
    function dsywpmCk(result){
        $.each(result,function(i,data){
            ...
            $('#dsywpm').append('<tr><td>'+NAME+'</td><td>'+VALUE+'</td></tr>');
        });
    }
}
...
function dsbcpm(){
    ...
    function dsbcpmCk(result){
        $.each(result,function(i,data){
            var NAME = data["cityName"];
            var VALUE = data["busicount"];
            $('#dsbcpm').append('<tr><td>'+NAME+'</td><td>'+VALUE+'</td></tr>');
        });
    }
}
...
function ssInput(){
    $("#search_text").keyup(function (event) {
            ...
            ajaxPostPre(getRootPath()+"/map/ss?departName="+old_value,ssInputCk)
